...
def timeify(string):
	...
	for m in range(0,len(a)):
		if not checknumeric(a[-m]):
			return -1
		#print("m : %i, val: %i" % (m,int(a[-m])))
		ret += 60**(len(a)-m-1) * int(a[-m])
	return ret
...
def toggleThread(event):
	...
	if not toggle and not threadactive:
		if not checknumeric(cspd.get()):
			...
			entryReset(settingsdict["regclickinterval"])
			return
		settingsdict["regclickinterval"] = float(timeify(cspd.get()))
		threadactive = True
		activity["text"] = "Status: Running"
		activity["background"] ="green"
		btn["text"] = "Stop"
		print("Set the regclickinterval to %f" % float(timeify(cspd.get())))
